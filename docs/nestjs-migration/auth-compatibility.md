...
| `GET /api/auth/profile-completion`                                                   | `GET /api/profile` and `GET /api/profile/eligibility`               | Profile completion is application data, not authentication protocol state. Use the protected Nest profile endpoints.                                    |
| Legacy `/api/security/2fa/**` setup, verify, backup-code, status, and disable routes | Better Auth two-factor plugin endpoints under `/api/auth/**`        | Use the native plugin request, challenge, backup-code, and session contracts. Nest deliberately exposes no compatibility `/api/security/2fa/**` routes. |
...
- `/api/profile/verify` ignores the legacy caller-controlled `force` bypass. Only an administrator with a satisfied Better Auth two-factor session can verify an eligible user; the server recalculates every criterion.
- Blocking and follow transitions use serializable transactions with bounded P2034 retry, symmetric relationship cleanup, recipient ownership, mutual-block concealment, and idempotent repeated requests.
- Security-event pages are owner-scoped and newest-first. Sensitive metadata keys and raw internal error messages are removed before persistence or response; request IP and user agent are bounded by a typed request-audit decorator.
...
## Profile media cutover gate

Profile and cover uploads write the validated Cloudinary asset before updating the user row. A database failure triggers a bounded provider deletion using the deterministic `aeko/users/<ownerId>/<purpose>` provider ID. If that compensation request also fails, the API preserves the original sanitized database error and never reports false success; production traffic cutover still requires an operational reconciliation procedure for these residual orphans.
