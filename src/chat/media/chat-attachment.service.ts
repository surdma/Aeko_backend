...
export class ChatAttachmentService {
  ...
  async upload(input: ChatUpload) {
    ...
    if (
      ...
      !ALLOWED.has(input.declaredMimeType)
    )
      throw new DomainError('VALIDATION_FAILED', 'Unsupported attachment');
    const detected = detectMime(input.bytes);
    if (detected !== input.declaredMimeType)
      throw new DomainError(
        'VALIDATION_FAILED',
        'Attachment type does not match its content',
      );
    const filename = input.filename
      .replace(/[^a-zA-Z0-9._-]/g, '_')
      .slice(0, 255);
    const uploaded = await this.media.uploadChatAttachment({
      ...input,
      filename,
    });
    return {
