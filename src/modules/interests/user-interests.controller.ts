...
import {
  type UserInterestIdsBody,
  userInterestIdsSchema,
...
export class UserInterestsController {
  ...
  private response(
    ...
    if (result.kind === 'user-not-found') {
      throw new NotFoundException(failure('User not found'));
    }
    if (result.kind === 'invalid-interest-ids') {
      throw new BadRequestException(
        failure('One or more interest IDs are invalid'),
      );
    }
    if (result.kind === 'interest-not-found') {
      throw new NotFoundException(
        failure('Interest not found in user interests'),
      );
    }
