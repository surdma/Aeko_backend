...
const twoFactorStateSchema = z
  .object({ isEnabled: z.boolean().default(false) })
  .passthrough();
...
export class AuthService {
  ...
  public async authenticate(
    ...
    try {
      ...
      const parsedTwoFactor = twoFactorStateSchema.safeParse(
        identity.twoFactorAuth,
      );
      const twoFactorEnabled =
        parsedTwoFactor.success && parsedTwoFactor.data.isEnabled;
      const user: AuthenticatedUser = {
        ...
        ...(parsed.data.partial === true && twoFactorEnabled
          ? { partialLogin: true }
          : {}),
      };
