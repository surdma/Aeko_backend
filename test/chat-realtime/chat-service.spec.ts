/* eslint-disable @typescript-eslint/require-await -- async test doubles implement persistence ports */
  ...
  it('authorizes sender and receiver before appending the authenticated command', async () => {
    ...
    const store: ChatStore = {
      appendMessage: async (command) => {
        ...
        expect(command.principalId).toBe(MEMBER_ID);
        return {
