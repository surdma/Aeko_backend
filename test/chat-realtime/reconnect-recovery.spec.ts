/* eslint-disable @typescript-eslint/require-await -- async test doubles implement recovery ports */
...
describe('ChatRecoveryService', () => {
  it('authorizes and returns messages after the sequence cursor in order', async () => {
    const calls: string[] = [];
    const rows = [2n, 3n, 4n].map((sequence) => ({
      id: String(sequence),
      chatId: 'c',
    ...
    const recovery = new ChatRecoveryService(
      {
        after: async (_c, sequence, limit) => {
          calls.push(`${sequence}:${limit}`);
          return rows;
        },
      },
      {
